...
mapview(twi,
  col.regions = RColorBrewer::brewer.pal(n = 9, "Blues"),
...
raster::writeRaster(dem,
  filename = file.path(tempdir(), "dem.sdat"),
  format = "SAGA"
)
params <- list(
  DEM = file.path(tempdir(), "dem.sgrd"),
  TWI = file.path(tempdir(), "twi.sdat")
)
rsaga.geoprocessor(
  lib = "ta_hydrology", module = "SAGA Wetness Index",
  param = params
)
# shortcut version
...
writeVECT(poly_1, vname = "poly_1", v.in.ogr_flags = "o")
writeVECT(poly_2, vname = "poly_2", v.in.ogr_flags = "o")
execGRASS("v.overlay",
  ainput = "poly_1", binput = "poly_2",
  output = "out_grass", operator = "and", flag = "overwrite"
)
# need to select either sf or sp
use_sf() 
